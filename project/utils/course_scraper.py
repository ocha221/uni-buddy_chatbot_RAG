...
class CourseScraper:
    ...
    def get_semester_links(self):
        ...
        return {
            key: list(value)
            for key, value in sorted(semester_courses.items(), key=lambda x: int(x[0]))
        }
            ...
            if header:
                ...
                if next_elem:
                    if next_elem.name == "ul":
                        return [
                            li.get_text().strip() for li in next_elem.find_all("li")
                        ]
    ...
    def scrape_courses(self, max_workers=None):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_semester = {
                executor.submit(
                    self._scrape_semester, semester, urls, timestamped_dir
                ): semester
        ...
        try:
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(Path(__file__).parent / "add_year.py"),
                    str(timestamped_dir),
                ]
            )
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(Path(__file__).parent / "change_to_int.py"),
                    str(timestamped_dir),
                ]
            )
