...
class FilterCourses(BaseModel):
    ...
    year: Optional[int] = Field(None, description="The academic year (e.g., 1, 2, 3, 4)")
...
class SearchNews(BaseModel):
    ...
    category: Optional[str] = Field(None, description="News category (internship, thesis, student, distinctions, events, vacancies, general)")
    limit: int = Field(10, description="Maximum number of results to return")

class GetRecentNews(BaseModel):
    """Get recent university news by category"""
    category: str = Field(..., description="News category (internship, thesis, student, distinctions, events, vacancies, general)")
...
def get_professor_courses(self,professor_service,name_service,professor_name: str) -> Dict:
        """Get courses taught by a professor"""
...
def search_courses(course_service, query: str, limit: int = 10) -> Dict:
    ...
    try:
        ...
        if results and results.get("ids") and results["ids"][0]:
            ...
            return {
                "success": False,
                "error": "No courses found matching your query."
            }
    except Exception as e:
...
def filter_courses(course_service, year: Optional[int] = None, semester: Optional[int] = None, limit: int = 10) -> Dict:
    """Filter courses by year, semester, or other criteria"""
    logger.info(f"Filtering courses with year: {year}, semester: {semester}, limit: {limit}")
...
def search_news(news_service, query: str, category: Optional[str] = None, limit: int = 10) -> Dict:
    """Search for university news and announcements"""
    logger.info(f"Searching news with query: '{query}', category: {category}, limit: {limit}")
    ...
    try:
        ...
        results = news_service.search_news(query, query_intent=query_intent, limit=limit)
        ...
        if has_results and has_documents and has_content:
            ...
            news_items = sorted(
                news_items,
                key=lambda x: -x.get("metadata", {}).get("date_epoch", 0)
            )[:limit]
...
def format_courses_for_data(results):
    ...
    for i in range(len(results["ids"][0])):
        course_info = {
            ...
            "course_code": results.get("metadatas", [[]])[0][i].get("course_code", "Unknown"),
            ...
            "professor": results.get("metadatas", [[]])[0][i].get("professor", "Unknown"),
            "content": results.get("documents", [[]])[0][i] if "documents" in results else "",
        }
