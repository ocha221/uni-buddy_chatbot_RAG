...
def professor_courses(professor_name):
    ...
    for course in courses:
        if course["metadata"]:
            click.echo(
                f"Course: {course['metadata'].get('title', 'Unknown')} ({course['course_code']})"
            )
            click.echo(
...
def interactive():
    ...
    click.echo(
        f"Course collection has {course_service.course_collection.count()} documents"
    )
    click.echo(
        f"Professor collection has {professor_service.professor_collection.count()} documents"
    )
    click.echo(
        f"Professor name mappings collection has {name_service.professor_names_collection.count()} documents"
    )
    ...
    click.echo(
        "- For smart professor search: 'find professor: query about a professor'"
    )
    ...
    while True:
        ...
        if user_input.startswith("search:"):
            ...
            if extracted_name:
                ...
                click.echo(
                    "Try being more specific about the professor you're looking for."
                )
            ...
            if extracted_name:
                click.echo(
                    f"Detected professor name: '{extracted_name}'. Showing courses:"
                )
                _print_professor_courses(extracted_name)
            else:
                click.echo(
                    "Unrecognized command format. Try 'search:', 'semester:', 'year:', 'professor:', 'find professor:', or 'add_variation'"
                )
