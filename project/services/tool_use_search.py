...
"""
        ...
        try:
            ...
            tool_logger.info(
                f"Successfully initialized LLM with {len(self.tools)} available tools"
            )
    ...
    def _execute_tool(self, tool_call):
        ...
        if tool_name == "getprofessorcourses":
            ...
            if (
                ...
                and "professor_name" in result
                and "courses" in result
            ):
                formatted_response = formatter.format_professor_courses(
                    result["professor_name"], result["courses"]
                )
            ...
            result = search_courses(
                self.course_service, args.get("query", ""), args.get("limit", 10)
            )
            # Format the search courses result
            if isinstance(result, dict) and "courses" in result:
                formatted_courses = formatter._format_courses_for_prompt(
                    result["courses"]
                )
            ...
            result = get_recent_news(
                self.news_service, args.get("category", "general"), args.get("limit", 5)
            )
    ...
    def process_message(self, message: str, thread_id: str = "default") -> Dict:
        ...
        debug_logger.info(
            f"Starting conversation processing with estimated ~{token_estimate} tokens"
        )
        ...
        try:
            ...
            if hasattr(ai_msg, "tool_calls") and ai_msg.tool_calls:
                ...
                for i, tool_call in enumerate(ai_msg.tool_calls):
                    tool_logger.info(
                        f"Processing tool call {i+1}/{num_tool_calls}: {tool_call['name']}"
                    )
                    ...
                    if truncated_result != tool_result:
                        debug_logger.warning(
                            "Tool result was truncated to reduce token count"
                        )
                    ...
                    token_estimate = self._estimate_messages_token_count(messages)
                    debug_logger.info(
                        f"Token estimate after tool call {i+1}: ~{token_estimate}"
                    )

                    # If we're approaching the limit, break
                    if (
                        token_estimate > 5000
                    ):  # Set a safe threshold below Groq's 6000 limit
                        debug_logger.warning(
                            f"Token count estimate (~{token_estimate}) approaching limit, breaking processing"
                        )
                        break
                ...
                debug_logger.info(
                    f"Final token estimate before response: ~{token_estimate}"
                )
                ...
                if token_estimate > 5000:
                    ...
                    token_estimate = self._estimate_messages_token_count(messages)
                    debug_logger.info(
                        f"Trimmed conversation to ~{token_estimate} tokens"
                    )
    ...
    def _maybe_truncate_result(self, result):
        ...
        if isinstance(result, dict):
            if (
                "courses" in result
                and isinstance(result["courses"], list)
                and len(result["courses"]) > 3
            ):
                debug_logger.info(
                    f"Truncating course list from {len(result['courses'])} to 3 items"
                )
                result["courses"] = result["courses"][:3]
                result["_truncated"] = True

            if (
                "news_items" in result
                and isinstance(result["news_items"], list)
                and len(result["news_items"]) > 3
            ):
                debug_logger.info(
                    f"Truncating news list from {len(result['news_items'])} to 3 items"
                )
            ...
            if "courses" in result and isinstance(result["courses"], list):
                for course in result["courses"]:
                    if (
                        "content" in course
                        and isinstance(course["content"], str)
                        and len(course["content"]) > 500
                    ):
                        course["content"] = (
                            course["content"][:500] + "... [content truncated]"
                        )
            ...
            if "news_items" in result and isinstance(result["news_items"], list):
                for item in result["news_items"]:
                    if (
                        ...
                        item["content"] = (
                            item["content"][:500] + "... [content truncated]"
                        )
