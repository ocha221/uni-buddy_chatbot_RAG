...
class ProfessorService:
        ...
        try:
            ...
            if exists:
                ...
                self.professor_collection.add(
                    documents=[document], ids=[prof_id], metadatas=[metadata]
                )
    ...
    def get_courses_by_professor(self, professor_name):
        ...
        try:
            ...
            if exact_match and len(exact_match["ids"]) > 0:
                ...
                logger.info(
                    f"Found {len(course_codes)} courses for professor {professor_name} via direct lookup"
                )
            ...
            logger.error(
                f"Error retrieving professor {professor_name}: {str(e)}\n continuing with full search"
            )
                    ...
                    matching_professors.append(
                        {
                            ...
                            "metadata": prof,
                            "document": (
                                all_professors["documents"][i]
                                if all_professors["documents"]
                                else None
                            ),
                        }
                    )
        ...
        try:
            ...
            if not all_course_codes:
                return None
            logger.info(
                f"Found {len(all_course_codes) if all_course_codes else 0} courses for professor {canonical_name}"
            )
        except Exception as e:
    ...
    def consolidate_professor_names(self, interactive=False):
        ...
        logger.info(
            f"Starting professor name consolidation at {datetime.now().strftime('%H:%M:%S')}"
        )
        ...
        logger.info(
            f"Found {len(unique_names)} unique professor names in {time.time() - start_time:.2f}s"
        )
        if interactive:
        ...
        if not self.name_service or not self.name_service.nlp_service:
            ...
            if interactive:
                print(
                    "[ERROR] NLP service not available, cannot perform name consolidation"
                )
            return 0
        ...
        try:
            embeddings = self.name_service.nlp_service.get_embeddings(
        ...
        for i, name in enumerate(unique_names):
            if i % 10 == 0:
                ...
                if interactive:
                    print(
                        f"\r[DEBUG] Processing name {i+1}/{len(unique_names)}...",
                        end="",
                        flush=True,
                    )
            ...
            for j in range(i + 1, len(unique_names)):
                ...
                try:
                    ...
                    similarity = self.name_service.nlp_service.calculate_similarity(
                    ...
                    logger.error(
                        f"Error calculating similarity between '{name}' and '{compare_name}': {str(e)}"
                    )
                    continue
        ...
        logger.info(
            f"Formed {len(name_groups)} groups in {time.time() - group_formation_start:.2f}s"
        )
        ...
        logger.info(
            f"Found {len(name_groups)} distinct professor entities from {len(unique_names)} name variants"
        )
        ...
        for group_index, group in enumerate(name_groups):
            ...
            if interactive:
                print(
                    f"\r[DEBUG] Processing group {group_index+1}/{len(name_groups)}...",
                    end="",
                    flush=True,
                )
            ...
            if len(group) == 1:
                single_name = group[0]
                logger.info(
                    f"Group {group_index+1} has only one name, generating embedding for '{single_name}'"
                )
                try:
                    embeddings = self.name_service.nlp_service.get_embeddings(
                        [single_name]
                    ...
                    logger.info(
                        f"Generated and stored embedding for single name '{single_name}'"
                    )
                    self.name_service.add_name_variation(single_name, single_name)
                except Exception as e:
                    logger.error(
                        f"Error generating embedding for single name '{single_name}': {str(e)}"
                    )
                continue
            ...
            logger.info(
                f"Selected canonical name: '{canonical_name}' for group: {group}"
            )
            try:
                embeddings = self.name_service.nlp_service.get_embeddings(
                    [canonical_name]
                ...
                logger.info(
                    f"Generated and stored embedding for canonical name '{canonical_name}'"
                )
            except Exception as e:
                logger.error(
                    f"Error generating embedding for canonical name '{canonical_name}': {str(e)}"
                )
            ...
            if interactive:
                ...
                print(
                    f"Name variations: {', '.join([v for v in group if v != canonical_name])}"
                )
                print("-" * 60)
                ...
                for variant in group:
                    ...
                    logger.info(
                        f"Found {len(courses) if courses else 0} courses for '{variant}' in {time.time() - variant_start:.2f}s"
                    )
                    print(f"\nCourses for '{variant}':")
                ...
                try:
                    ...
                    if confirm.lower() == "n":
                        logger.info(
                            f"User skipped consolidation of group {group_index+1}"
                        )
                    ...
                    print(
                        f"[ERROR] Input error: {str(e)}. Continuing with consolidation."
                    )
            ...
            for variant in group:
                if variant != canonical_name:
                    ...
                    logger.info(
                        f"Attempting to add variation '{variant}' for canonical name '{canonical_name}'"
                    )

                    var_start = time.time()
                    variation_added = self.name_service.add_name_variation(
                        canonical_name, variant
                    )
                    logger.info(
                        f"Added variation in {time.time() - var_start:.2f}s: {variation_added}"
                    )

                    if interactive:
                        print(
                            f"Consolidating courses from '{variant}' to '{canonical_name}'..."
                        )

                    consol_start = time.time()
                    try:
                        success = self._consolidate_professor_courses(
                            variant, canonical_name
                        )
                        if success:
                            consolidations_succeeded += 1
                        logger.info(
                            f"Course consolidation from '{variant}' to '{canonical_name}' in {time.time() - consol_start:.2f}s: {success}"
                        )
                    except Exception as e:
                    ...
                    if interactive and success:
                        print(
                            f"Successfully consolidated courses from '{variant}' to '{canonical_name}'"
                        )
        ...
        logger.info(
            f"Consolidation summary: {consolidations_succeeded}/{consolidations_attempted} successful"
        )
        ...
        if interactive:
            ...
            print(
                f"[SUMMARY] {consolidations_succeeded}/{consolidations_attempted} successful consolidations"
            )
    ...
    def rebuild_professor_data(self):
        ...
        self.collections.reset_collections(
            reset_courses=False, reset_professors=True, reset_name_mappings=False
        )
        self.professor_collection = self.collections.get_professor_collection()

        # Get all courses
        courses = self.course_service.course_collection.get(
            ...
            try:
                ...
                for line in doc.split("\n"):
                    if line.startswith("Instructors:"):
                        ...
                        instructors = [
                            name.strip() for name in instructor_text.split(",")
                        ]
                        break
        ...
        logger.info(
            f"Processing {len(instructor_course_pairs)} instructor-course pairs..."
        )
        ...
        logger.info(
            f"Successfully added {success_count} professor-course relationships"
        )
