...
class ChatService:
    ...
    def get_session(self, session_id):
        ...
        if session and (datetime.now() - session.last_activity).total_seconds() > (self.session_expiry_minutes * 60):
    ...
    def process_message(self, session_id, message, **services):
        ...
        if session.get_context("awaiting_clarification"):
            ...
            if is_followup:
                ...
                response_data, response_message, query_type = self.nlp_service.process_unified_query(
                    message,
                    query_intent,
                    **services
                )
                ...
                self._update_session_context(session, query_type, response_data, query_intent)
    ...
    def _is_followup_question(self, message, session):
        ...
        followup_phrases = [
            ...
            "what else", "can you elaborate", "show me", "what are"
        ]
        ...
        has_pronoun = any(word in message_lower.split() for word in ["it", "they", "them", "those", "that", "this", "these"])
        is_short_query = len(message.split()) <= 5
        
        #? Check for specific follow-up patterns
        has_course_reference = "course" in message_lower or "class" in message_lower or "subject" in message_lower
        has_professor_reference = "professor" in message_lower or "teacher" in message_lower or "instructor" in message_lower
        
        #? Look for references to previous context
        references_previous = False
        
        if active_professor and (
            active_professor.lower() in message_lower or 
            has_professor_reference or 
            (has_pronoun and last_intent == IntentType.PROFESSOR_COURSES)
        ):
            references_previous = True
        
        if active_courses and (
            has_course_reference or 
            (has_pronoun and (last_intent == IntentType.COURSE_SEARCH or last_intent == IntentType.COURSE_FILTERING))
        ):
            references_previous = True
        
        return (has_followup_phrase or has_pronoun or is_short_query) and references_previous
    ...
    def _process_followup(self, session, message, **services):
        ...
        if active_professor and last_intent == IntentType.PROFESSOR_COURSES:
            ...
            if "semester" in message.lower() or "year" in message.lower():
                ...
                if time_info and active_courses:
                    ...
                    return None, f"Professor {active_professor} doesn't teach any courses in the specified time period.", "no_results"
        ...
        elif last_intent in [IntentType.COURSE_SEARCH, IntentType.COURSE_FILTERING] and active_courses:
            #? Follow-up about courses previously searched
            course_service = services.get("course_service")
            
            if "professor" in message.lower() or "instructor" in message.lower() or "who teaches" in message.lower():
                #? User is asking about who teaches these courses
                if active_courses and len(active_courses) > 0:
                    course_codes = [course.get("course_code") for course in active_courses if course.get("course_code")]
                    
                    professor_info = {}
                    if course_service and hasattr(course_service, "get_professors_by_courses"):
                        professor_info = course_service.get_professors_by_courses(course_codes)
                    ...
                    response_message = f"Here's information about who teaches these courses:\n\n"
            ...
            if active_courses and len(active_courses) > 0:
                ...
                for course in active_courses:
                    ...
                    if code in message.lower() or any(word in message.lower() for word in title_words if len(word) > 3):
                        #? User is likely asking about this specific course
                        if course_service:
                            detailed_course = course_service.get_course(course.get("course_code"))
                            ...
                            if detailed_course:
                                ...
                                return context_data, response_message, IntentType.COURSE_SEARCH
                ...
                response_message = self.nlp_service.generate_response(
                    last_intent,
                    context_data,
                    message  #? Use the original message
                )
    ...
    def _update_session_context(self, session, query_type, response_data, query_intent=None):
        ...
        if query_type == IntentType.PROFESSOR_COURSES and response_data:
            if "professor_name" in response_data:
                session.update_context("active_professor", response_data["professor_name"])
        ...
        elif query_type in [IntentType.COURSE_SEARCH, IntentType.COURSE_FILTERING] and response_data:
            if "courses" in response_data:
                session.update_context("active_courses", response_data["courses"])
