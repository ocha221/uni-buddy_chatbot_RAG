...
class RefreshService:
    ...
    def find_latest_data_directory(
        self, base_path, pattern=r"(\d{2}_\d{2}_\d{4})_scraped_on"
    ):
        ...
        if latest_dir:
            logger.info(
                f"Found latest data directory: {latest_dir} (from {latest_date})"
            )
        else:
    ...
    def retry_operation(self, operation_func, items, max_retries=3, delay=5):
        ...
        while failed_items and retry_count < max_retries:
            retry_count += 1
            logger.info(
                f"Retry attempt {retry_count}/{max_retries} for {len(failed_items)} failed items"
            )
            ...
            for item in failed_items:
                try:
                    logger.info(
                        f"Retrying item: {getattr(item, 'course_code', getattr(item, 'id', str(item)))}"
                    )
                    if operation_func(item):
                        successful_items.append(item)
                    else:
        ...
        if failed_items:
            logger.warning(
                f"After {max_retries} retries, {len(failed_items)} items still failed"
            )

        return successful_items, failed_items

    def refresh_courses(
        self, base_path="course_data", skip_professors=False, reset=True, max_retries=3
    ):
        ...
        if reset:
            ...
            self.course_service.collections.reset_collections(
                reset_courses=True, reset_professors=False, reset_name_mappings=False
            )
        ...
        def add_course_operation(course):
            if skip_professors:
                ...
                self.course_service.course_collection.add(
                    documents=[document], ids=[course.course_code], metadatas=[metadata]
                )

                time.sleep(self.course_service.EMBEDDING_DELAY)

                logger.info(
                    f"Added course {course.course_code} to database (skipped professor data)"
                )
                return True
            else:
                return self.course_service.add_course(course)

        successful, failed = self.retry_operation(
            add_course_operation, courses, max_retries=max_retries
        )

        if failed:
            logger.warning(
                f"Failed to import {len(failed)} courses: {[getattr(c, 'course_code', 'Unknown') for c in failed]}"
            )

        logger.info(
            f"Successfully imported {len(successful)} out of {len(courses)} courses"
        )
    ...
    def refresh_news(self, base_path="news_data", reset=True, max_retries=3):
        ...
        if reset:
            ...
            self.news_service.news_collection = (
                self.news_service.collections.get_news_collection()
            )
        ...
        successful, failed = self.retry_operation(
            add_news_operation, news_items, max_retries=max_retries
        )

        if failed:
            logger.warning(
                f"Failed to import {len(failed)} news items: {[getattr(n, 'id', 'Unknown') for n in failed]}"
            )

        logger.info(
            f"Successfully imported {len(successful)} out of {len(news_items)} news items"
        )
        return len(successful)

    def refresh_all(
        self,
        course_path="course_data",
        news_path="news_data",
        reset=True,
        max_retries=3,
    ):
        """Refresh all collections with latest data"""
        course_count = self.refresh_courses(
            course_path, skip_professors=True, reset=reset, max_retries=max_retries
        )
        ...
        return {"courses": course_count, "news": news_count, "professors": prof_count}
