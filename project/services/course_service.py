...
class CourseService:
        ...
        try:
            ...
            if self.professor_service and course.instructors and not skip_professors:
                ...
                for instructor in instructors:
                    self.professor_service.add_professor(
                        instructor, course.course_code, course.title
                    )
    ...
    def filter_courses(self, filters, limit=10):
        ...
        try:
            results = self.course_collection.query(
                query_texts=[""], where=filters, n_results=limit
            )
            return results
        except Exception as e:
    ...
    def batch_import_courses(self, directory, skip_professors=False):
        ...
        logger.info(
            f"Successfully imported {success_count} out of {len(course_files)} courses"
        )
